...
- 三種操作模式：**Build**（僅建置）、**Upload**（僅上傳）、**Build & Upload**（一鍵全流程）。
- 可設定 **Build Output Path**（建置輸出路徑），儲存於 config asset（支援絕對路徑或相對於專案根目錄的路徑）。建置時先輸出至暫存目錄（atomic temp-swap），失敗時不影響原有輸出。
...
- **Unity 6+ Build Profile** 支援：可選擇在建置前啟用指定的 Build Profile asset。
- 認證與 App Settings 區段在欄位填妥後自動收折。
...
| **Build Output Path** | Unity 建置輸出目錄的絕對路徑或相對於專案根目錄的路徑。同時作為 Steam 上傳的 depot 內容根目錄 |
...
| 按鈕 | 說明 |
|------|------|
| **Build** | 執行 `BuildPipeline.BuildPlayer`，輸出至設定的 Build Output Path。需先填寫輸出路徑。 |
| **Upload** | 根據現有建置產生 VDF 腳本並啟動 SteamCMD。需要輸出目錄中已存在可執行檔。 |
| **Build & Upload** | 依序執行建置與上傳。所有欄位填妥且設定輸出路徑後才可用。 |

點擊 **Build & Upload** 後依序執行：

| 階段 | 說明 |
|------|------|
| 驗證設定 | 檢查欄位完整性、路徑合法性、non-ASCII 字元 |
| Unity 建置 | 呼叫 `BuildPipeline.BuildPlayer`，使用當前 Build Settings（Unity 6+ 可選 Build Profile）。先輸出至暫存目錄，成功後取代正式輸出目錄 |
